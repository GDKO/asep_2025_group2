...
USER_DATA_FILE = os.path.join(get_user_data_dir(), "user_data_g2.json")
NUM_QUESTIONS = 25
...
class QuizApp(tk.Tk):
    ...
    def select_weighted_questions(self, user_adj_weights, k=NUM_QUESTIONS):
        ...
        weights = np.array([user_adj_weights.get(q["question"], q.get("adj_weight", 1)) for q in self.questions], dtype=float)
        weights /= weights.sum()
    ...
    def create_login_screen(self):
        ...
        tk.Label(self.login_frame, text="Asep Quiz Group 2", font=("Arial", 16)).pack(pady=20)
    ...
    def start_quiz(self):
        ...
        if username not in self.user_data:
            ...
            weights = {q["question"]: 100 for q in self.questions}
            times_chosen = {q["question"]: 0 for q in self.questions}
            correct_answers = {q["question"]: 0 for q in self.questions}
            adj = {q["question"]: 1 for q in self.questions}
            adj_weights = {q["question"]: 100 for q in self.questions}
        ...
        for q in self.questions:
            self.user_info["adj"][q["question"]] += 1
            self.user_info["adj_weights"][q["question"]] = self.user_info["adj"][q["question"]] * self.user_info["weights"][q["question"]]
    ...
    def check_answer(self):
        ...
        times_chosen = self.user_info["times_chosen"].get(q["question"], 0) + 1
        correct_answers = self.user_info["correct_answers"].get(q["question"], 0)
